...
class ParserTests(unittest.TestCase):
    ...
    def test_help_includes_output_review_and_liability_notice(self):
        help_text = make_my_resume_fit.build_parser().format_help()

        self.assertIn("Outputs are generated by automation", help_text)
        self.assertIn("must be reviewed before use", help_text)
        self.assertIn("warranty and liability terms", help_text)


class DocumentationTests(unittest.TestCase):
    def test_readme_states_codex_is_mandatory(self):
        readme = Path("README.md").read_text(encoding="utf-8")

        self.assertIn("## Requirements", readme)
        self.assertIn("configured and working local `codex` command is mandatory", readme)
        self.assertIn("installed, authenticated, and able to run", readme)

    def test_readme_includes_liability_disclaimer(self):
        readme = Path("README.md").read_text(encoding="utf-8")

        self.assertIn("## Legal", readme)
        self.assertIn("[DISCLAIMER.md](DISCLAIMER.md)", readme)
        self.assertIn("[PRIVACY.md](PRIVACY.md)", readme)
        self.assertIn("[THIRD_PARTY_CONTENT.md](THIRD_PARTY_CONTENT.md)", readme)
        self.assertIn("Review every output carefully", readme)

    def test_disclaimer_limits_generated_output_liability_and_guarantees(self):
        disclaimer = Path("DISCLAIMER.md").read_text(encoding="utf-8")

        self.assertIn("not liable for any claims, damages, losses", disclaimer)
        self.assertIn("Generated resumes and metadata may contain errors", disclaimer)
        self.assertIn("does not guarantee", disclaimer)
        self.assertIn("not legal advice", disclaimer)

    def test_privacy_notice_covers_resume_data_and_external_processing(self):
        privacy = Path("PRIVACY.md").read_text(encoding="utf-8")

        self.assertIn("Resumes often contain personal information", privacy)
        self.assertIn("temporary run directory", privacy)
        self.assertIn("may be processed by external services", privacy)
        self.assertIn("permission to use", privacy)

    def test_third_party_content_notice_covers_job_posting_access_rules(self):
        notice = Path("THIRD_PARTY_CONTENT.md").read_text(encoding="utf-8")

        self.assertIn("job-offer URLs", notice)
        self.assertIn("website terms", notice)
        self.assertIn("bypass access controls", notice)
        self.assertIn("Review generated outputs", notice)
...
class RenderingTests(unittest.TestCase):
    ...
    def test_rendered_prompt_rejects_guaranteed_outcome_claims(self):
        rendered = make_my_resume_fit.render_template(
            make_my_resume_fit.load_template(),
            input_resume="orig.tex",
            job_offers=["https://example.com/a"],
        ...
        self.assertIn("Do not state or imply", rendered)
        self.assertIn("guaranteed to pass applicant", rendered)
        self.assertIn("not promise outcomes", rendered)

    def test_rendered_prompt_requires_human_verification_warnings(self):
        rendered = make_my_resume_fit.render_template(
            make_my_resume_fit.load_template(),
            input_resume="orig.tex",
            job_offers=["https://example.com/a"],
        ...
        self.assertIn("needs human verification before submission or sharing", rendered)
